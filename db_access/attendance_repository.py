...
class AttendanceRepository(db_access):
    ...
    def add_attendance(self, student_id):
        if self.check_student_attendance(student_id):
            return
    ...
    def get_all_attendances_in_class(self, class_name, date=""):
        ...
        for student in students:
            ...
            if student[0] in attendances:
                ...
                status = "ON TIME" if self.str_to_mins(enter_time) <= class_enter_time else "LATE"
    ...
    def get_student_status(self, student_id):
        ...
        class_enter_time = self.query("SELECT enter_time from classes WHERE class_name=?", (class_name))[0]
